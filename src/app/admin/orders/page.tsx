  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(order =>
        ...
        (order.studentName || '').toLowerCase().includes(lowerTerm)
      )
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div className="relative">
          ...
          <input 
            type="text" 
            placeholder="Sipariş No, Öğrenci, Müşteri veya E-posta ara..." 
            value={searchTerm}
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-gray-600">Öğrenci</th>
            ...
            currentData.map((order) => (
              ...
              >
                ...
                <div className="flex justify-between items-center text-xs text-gray-500">
                  <span>
                    {order.studentName ? `Öğrenci: ${order.studentName}` : (order.guestCustomerName ?? order.user?.name ?? '—')}
                  </span>
                  <span>
                    {new Date(order.createdAt).toLocaleDateString('tr-TR')}{' '}
                    {new Date(order.createdAt).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}
